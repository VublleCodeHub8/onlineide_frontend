...
function ForgotPasswordForm({ onClose }) {
  ...
  async function onSubmit(values) {
    try {
      setLoading(true);
      // Prepare payload - we only need email and newPassword
      const payload = {
        email: values.email,
        newPassword: values.newPassword
      };
      
      const res = await fetch(`${import.meta.env.VITE_API_URL}/auth/changepass`, {
        method: "POST",
  ...
  return (
    <>
      ...
      <motion.div 
        ...
        className={`bg-[#1a1a1a] rounded-2xl p-8 w-[500px] border-2 border-[#333] shadow-2xl ${showForgotPassword ? 'blur-sm' : ''}`}
      >
      <Form {...form}>
